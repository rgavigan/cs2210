...
public class Graph implements GraphADT {
    ...
    public Graph(int n) {
        ...
        for (int i = 0; i < numVertices; i++) {
            // Create new node and add to nodes list
            Node newNode = new Node(i);
            // Add empty ArrayList of edges to index i in edges arraylist
    ...
    public void addEdge(Node u, Node v, String edgeType) throws GraphException {
        ...
        // check if edge already exists in adjacency list by checking if edge is in edges arraylist
        for (int i = 0; i < edges.size(); i++) {
            // Loop through arraylist of edges
            for (int j = 0; j < edges.get(i).size(); j++) {
                Edge compare = edges.get(i).get(j);
    ...
    public Edge getEdge(Node u, Node v) throws GraphException {
        ...
        // Loop through adjacency list to check if edge exists between node u and node v
        for (int i = 0; i < edges.size(); i++) {
            // Loop through arraylist of edges
            for (int j = 0; j < edges.get(i).size(); j++) {
                Edge compare = edges.get(i).get(j);
